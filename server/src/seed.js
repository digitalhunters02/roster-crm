...
async function main() {
  ...
  const alreadySeeded = (await get(`SELECT COUNT(*) AS n FROM clients`)).n > 0;
  if (alreadySeeded) {
  ...
  for (const r of clientRows) {
    clientIds.push((await run(`INSERT INTO clients (company_name, industry, contact_name, contact_email, contact_phone, address, owner_user_id, status, notes)
    VALUES (@company_name, @industry, @contact_name, @contact_email, @contact_phone, @address, @owner_user_id, @status, @notes) RETURNING id`, r)).rows[0].id);
  }
  ...
  for (const j of jobSeed) {
    jobIds.push((await run(`INSERT INTO jobs (client_id, title, employment_type, pay_range, location, status, owner_user_id, opened_date, notes)
  ...
  for (let i = 0; i < candidateSeed.length; i++) {
    ...
    candidateIds.push((await run(`INSERT INTO candidates (name, email, phone, current_title, current_employer, skills, resume_summary, source, owner_user_id, created_at)
  ...
  for (const s of submissionSeed) {
    submissionIds.push((await run(`INSERT INTO submissions (candidate_id, job_id, stage, owner_user_id, submitted_date, notes)
  ...
  for (const i of interviewSeed) {
    await run(`INSERT INTO interviews (submission_id, interviewer_name, scheduled_at, type, status, notes)
  ...
  const placementIds = [];
  for (const p of placementSeed) {
    placementIds.push((await run(`INSERT INTO placements (submission_id, candidate_id, client_id, job_id, start_date, end_date, pay_rate, bill_rate, placement_fee, status)
  ...
  for (const t of timesheetSeed) {
    await run(`INSERT INTO timesheets_invoices (placement_id, period_start, period_end, hours, amount, status)
  ...
  for (const a of activitySeed) {
    await run(`INSERT INTO activities (type, subject, related_type, related_id, owner_user_id, occurred_at, notes)
  ...
  for (const r of automationRows) {
    await run(`INSERT INTO automations (name, trigger_desc, action_desc, active, runs_30d)
    VALUES (@name, @trigger_desc, @action_desc, @active, @runs_30d)`, r);
  }
